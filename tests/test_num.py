...
class TestPointsInPolygon:
    ...
    def test_points_in_polygon_complex_shape(self):
        ...
        polygon = np.array([[0, 0], [1, 0], [1, 1], [2, 1], [2, 2], [0, 2]], dtype=np.float64)
...
class TestPeakFinding:
    ...
    def test_find_signed_peaks_2d_parallel_vs_serial(self):
        ...
        # Serial version
        minima_serial, maxima_serial = snum.find_signed_peaks_2d(data, wx, wy, paral=False)

        # Parallel version
        minima_parallel, maxima_parallel = snum.find_signed_peaks_2d(data, wx, wy, paral=True)
...
def test_small_data_peak_finding():
    ...
    # 3x3 array
    data = np.array([[1, 1, 1], [1, 5, 1], [1, 1, 1]], dtype=np.float64)  # Peak at center

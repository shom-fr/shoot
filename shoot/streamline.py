...
def psi(u,v): 
    ...
    #get lat, lon 
    lat, lon = xcoords.get_lat(u), xcoords.get_lon(u)
    ...
    x = xgeo.deg2m(dlon2d, lat2d.values)/1e3
    y = xgeo.deg2m(dlat2d)/1e3 
    ...
    cx2 = cumulative_trapezoid(v[cj,ci::-1], x[cj,ci::-1])#-cumulative_trapezoid(v[cj,ci::-1], x[cj,ci::-1])
    ...
    psi_xy = np.block([[psi_xy22[::-1, ::-1], psi_xy12[::-1, :]],
                       [psi_xy21[:, ::-1], psi_xy11]])
    ...
    # Concatenate the 4 parts (NE, SE, NO, SO)
    psi_yx = np.block([[psi_yx22[::-1, ::-1], psi_yx12[::-1, :]],
                      [psi_yx21[:, ::-1], psi_yx11]])

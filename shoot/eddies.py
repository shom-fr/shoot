...
def find_eddy_centers(u, v, window, dx=None, dy=None):
    ...
    -------
    xarray.Dataset
        With "lon" and "lat" coordinates and "gi" and "gi" grid indices
    """
    assert u.ndim == 2, "It only works for 2d arrys for the moment"
...
def detect_eddies(
    ...
    centers = find_eddy_centers(u, v, window_center, dx=dxm, dy=dym)

    # Fit window
    wx, wy = sgrid.get_wx_wy(window_fit, dxm, dym)
    wx2 = wx // 2
    wy2 = wy // 2

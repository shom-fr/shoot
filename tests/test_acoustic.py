...
def typical_sound_speed_profile():
    ...
    depths = np.array([0, 10, 20, 50, 100, 200, 500, 1000, 2000])
    # Typical profile: surface channel, deep sound channel minimum, then increase
    sound_speeds = np.array([1520, 1515, 1510, 1505, 1480, 1475, 1490, 1510, 1530])
...
def sample_cs_dataset():
    ...
    ds = xr.Dataset({
        ...
        'depth': (['s_rho', 'eta_rho', 'xi_rho'], 
                 np.broadcast_arrays(depths[:, None, None], 
                                   np.ones((len(lats), len(lons))))[0])
    })
...
class TestAcousEddyClass:
    ...
    def setup_method(self):
        ...
        # Mock depth vector
        self.mock_anomaly.depth_vector = xr.DataArray(
            np.linspace(0, 1000, 50), dims=['depth']
        )
        ...
        self.mock_anomaly.profils_inside = self.mock_anomaly.profils_inside.assign_coords(
            lon_rho=(['nb_profil'], lons),
            lat_rho=(['nb_profil'], lats)
        )

...
#. Loop on eddy centers and initialize an :class:`~shoot.eddies.RawEddy2D` instance from the gridded fields and eddy properties:
   ...
   #. Estimate local eddy pseudo-SSH from currents with :func:`~shoot.fit.fit_ssh_from_uv` if not already provided. It is generally useful for subsurface detections.
   #. Compute SSH closed contours with :func:`~shoot.contours.get_closed_contours` and store them in :attr:`~shoot.eddies.RawEddy2D.contours`.

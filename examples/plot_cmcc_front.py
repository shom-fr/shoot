...
import gsw
import contourpy
from shoot.front.algos import (
    canny_front,
    compute_gradients,
    my_canny_from_gradients,
    boa_wrapper,
    wrapper_cca,
)
...
# ds = xr.open_dataset(os.path.join(path, "temp_cur_zos_cmcc_ana_ionian_proteion2024.nc"))
ds = xr.open_dataset(os.path.join(path, "cmcc_u_v_t_s_3D_oct2025.nc"))
ds
...
plt.title("MCP")
cb = plt.pcolormesh(
    dpc.lon, dpc.lat, mcp, cmap="Spectral_r", vmin=50, vmax=500
)
plt.colorbar()
...
plt.title("ECS")
cb = plt.pcolormesh(
    dss.longitude, dss.latitude, ecs, cmap="Spectral_r", vmin=00, vmax=50
)
plt.pcolormesh(dss.longitude, dss.latitude, canny, cmap="gray_r", alpha=0.2)
plt.colorbar(cb)
...
plt.title("MCP")
cb = plt.pcolormesh(
    dss.longitude, dss.latitude, mcp, cmap="cmo.dense", vmin=100, vmax=400
)
plt.pcolormesh(dss.longitude, dss.latitude, canny, cmap="gray_r", alpha=0.2)
plt.colorbar(cb)
...
plt.figure()
cb = plt.pcolormesh(
    dss.longitude, dss.latitude, dss.thetao, cmap="cmo.thermal"
)
...
plt.pcolormesh(
    dss.longitude, dss.latitude, boa_skeleton, cmap="gray_r", alpha=0.5
)
plt.colorbar(cb)
...
plt.title("ECS")
cb = plt.pcolormesh(
    dss.longitude, dss.latitude, s_ecs, cmap="Spectral_r", vmin=00, vmax=50
)
...
plt.title("MCP")
cb = plt.pcolormesh(
    dss.longitude, dss.latitude, s_mcp, cmap="Spectral_r", vmin=100, vmax=400
)
plt.pcolormesh(dss.longitude, dss.latitude, cca, cmap="gray_r", alpha=0.5)
plt.colorbar(cb)
